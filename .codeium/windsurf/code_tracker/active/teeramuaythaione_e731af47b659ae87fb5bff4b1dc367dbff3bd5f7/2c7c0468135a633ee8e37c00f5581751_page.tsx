...
export default async function Home() {
  ...
  // Fetch initial data
  const [upcomingEvents, featuredFighters, featuredCourses, featuredVenues, featuredPosts, initialVenuesByType] = await Promise.all([
    ...
    api.venue.getByVenueType({ limit: 100 }) // Get all venues by type, don't filter by featured to ensure we have venues
  ]);
  
  // Use a mutable variable for venues by type
  let venuesByType = initialVenuesByType;
  
  // Seed venues if none are found
  if (!venuesByType ?? Object.keys(venuesByType.groupedVenues).length === 0) {
    try {
      // Try to run the seed script
      await fetch('/api/seed-venues', { method: 'POST' });
      // Refetch venues after seeding
      const refreshedVenues = await api.venue.getByVenueType({ limit: 100 });
      if (refreshedVenues) {
        venuesByType = refreshedVenues;
      }
    } catch (error) {
            ...
            <div className="space-y-12">
              ...
              {Object.entries(venuesByType.groupedVenues)
                ...
                <div key={venueType} className="space-y-4">
                  ...
                  {(venues as VenueWithTypes[]).length > 6 && (
...
��"(e731af47b659ae87fb5bff4b1dc367dbff3bd5f72Pfile:///c:/work/projects/newpro/teeonedWinsurf/teeramuaythaione/src/app/page.tsx:?file:///c:/work/projects/newpro/teeonedWinsurf/teeramuaythaione