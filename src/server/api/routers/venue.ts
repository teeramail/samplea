...
export const venueRouter = createTRPCRouter({
    ...
    .input(
      z
        .object({
          ...
          sortField: z
            .enum(["name", "region", "address", "featured", "updatedAt"])
            .default("updatedAt"),
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        switch (sortField) {
          case "name":
            orderBy =
              sortDirection === "asc" ? asc(venues.name) : desc(venues.name);
            break;
          case "featured":
            orderBy =
              sortDirection === "asc"
                ? asc(venues.isFeatured)
                : desc(venues.isFeatured);
            break;
          case "updatedAt":
            orderBy =
              sortDirection === "asc"
                ? asc(venues.updatedAt)
                : desc(venues.updatedAt);
            break;
          default:
  ...
  delete: publicProcedure // TODO: Change to adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        await ctx.db.delete(venues).where(eq(venues.id, input.id));
  ...
  getByVenueType: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        const venuesWithTypes = await Promise.all(
          venuesList.map(async (venue) => {
            const venueTypeLinks =
              await ctx.db.query.venueToVenueTypes.findMany({
            ...
            return {
              ...
              primaryType:
                venueTypeLinks.find((link) => link.isPrimary)?.venueType ??
                venueTypeLinks[0]?.venueType,
            };
          }),
        );
        ...
        const venuesWithNoTypes = venuesWithTypes.filter(
          (venue) => !venue.venueTypes.length,
        );
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to fetch venues by type",
        });
      }
    }),
  ...
  getAllIds: publicProcedure.query(async ({ ctx }) => {
    try {
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to fetch venue IDs",
      });
    }
  }),
});
