...
async function generateUniqueSlug(
  ...
  while (true) {
    const conditions = [eq(trainingCourses.slug, uniqueSlug)];
  ...
  update: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (updateData.skillLevel !== undefined)
        dataToUpdate.skillLevel = updateData.skillLevel;
      if (updateData.duration !== undefined)
        dataToUpdate.duration = updateData.duration;
      if (updateData.scheduleDetails !== undefined)
        dataToUpdate.scheduleDetails = updateData.scheduleDetails;
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input?.instructorId) {
        whereConditions.push(
          eq(trainingCourses.instructorId, input.instructorId),
        );
      }
