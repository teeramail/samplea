...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      "Received ChillPay background notification with data:",
    ...
    if (!process.env.CHILLPAY_MD5_SECRET) {
      console.error(
        "[Webhook] Missing CHILLPAY_MD5_SECRET environment variable",
      );
    ...
    if (calculatedChecksum !== receivedChecksum) {
      console.error(
        "[Webhook] Checksum validation failed - Rejecting notification",
      );
      return NextResponse.json(
    ...
    console.log(
      `[Webhook] Updated booking ${booking.id} payment status to ${newStatus}`,
    );
