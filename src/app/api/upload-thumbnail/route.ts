...
export async function POST(req: Request) {
  try {
    ...
    if (!(file instanceof File)) {
      return NextResponse.json({ error: "No thumbnail file uploaded" }, { status: 400 });
    }
    if (file.size > 30 * 1024) {
      return NextResponse.json({ error: "Thumbnail must be less than 30KB" }, { status: 400 });
    }
    ...
    const result = await uploadImages([file], entityType as string || "venue-thumbnail", entityId as string | undefined);
    if (!result.success) {
    ...
    return NextResponse.json({ error: "Failed to process thumbnail upload", details: message }, { status: 500 });
  }
}
