...
export default function CreateInstructorPage() {
  ...
  const createInstructor = api.instructor.create.useMutation({
    onSuccess: (data) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({});
    setUploadStatus("Creating instructor...");
    ...
    try {
      ...
      // Submit to API
      createInstructor.mutate(validatedData);
    } catch (error) {
      setIsSubmitting(false);
      setUploadStatus("");
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <h1 className="mb-6 text-2xl font-bold">Create New Instructor</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-3">
          ...
          >
            {isSubmitting ? uploadStatus || "Creating..." : "Create Instructor"}
          </button>
        </div>
      </form>
    </div>
