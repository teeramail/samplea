...
function hasItems(
  data: Venue[] | VenuesResponse | undefined,
): data is VenuesResponse {
...
export default function AdminVenuesPage() {
  ...
  const [sortField, setSortField] = useState<
    "name" | "region" | "address" | "featured" | "updatedAt"
  >("updatedAt");
  ...
  // Handle column sort
  const handleSort = (
    field: "name" | "region" | "address" | "featured" | "updatedAt",
  ) => {
    if (sortField === field) {
  ...
  return (
    <div>
      ...
      <div className="space-y-4 lg:hidden">
        {venues.map((venue) => (
          <div key={venue.id} className="rounded-lg bg-white p-4 shadow-sm">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th
                scope="col"
                className="cursor-pointer px-6 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 hover:bg-gray-100"
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {venues.map((venue) => (
              <tr
                key={venue.id}
                className={`${venue.isFeatured ? "bg-indigo-50" : ""} cursor-pointer hover:bg-gray-50`}
      ...
      {venues.length === 0 && (
        <div className="rounded-lg bg-white py-8 text-center shadow-sm">
          ...
          {!searchQuery && showFeaturedOnly && (
            <p className="mt-1 text-sm text-gray-500">
              No featured venues yet. Toggle the switch to feature a venue.
            </p>
          )}
          {!searchQuery && !showFeaturedOnly && (
            <p className="mt-1 text-sm text-gray-500">
