  ...
  useEffect(() => {
    if (fighter && !isFighterLoading) {
      setFormData({
        ...
        biography: (fighter as any).biography || "",
        thumbnailUrl: (fighter as any).thumbnailUrl || "",
        imageUrl: fighter.imageUrl || "",
        imageUrls: (fighter as any).imageUrls || [],
      ...
      const existingImageUrls = (fighter as any).imageUrls || [];
      if (existingImageUrls.length > 0) {
        const existingImages = existingImageUrls.map((url: string, index: number) => ({
          url,
          originalFilename: `Existing fighter image ${index + 1}`
        }));
        setFighterImages(existingImages);
      }
